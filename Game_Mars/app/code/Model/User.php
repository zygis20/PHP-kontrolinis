...
class User extends ModelAbstract
{
    ...
    public static function isNameUnic($name)
    {
        $db = new Db();
        $result = $db->select()->from(self::TABLE_NAME)->where(self::NAME_COLUMN, $name)->get();
        return empty($result) ? true : false;
    }
    ...
    public static function isValidName($user_name)
    {
        $db = new Db();
        $result = $db->select()
            ->from(self::TABLE_NAME)
            ->where(self::NAME_COLUMN, $user_name)
            ->get();
        if (!empty($result)) {
            return true;
        }
    ...
    {
        $db = new Db();
        $user = $db->select()->from(self::TABLE_NAME)->where(self::NAME_COLUMN, $user_name)->getOne();
        $this->load($user['id']);
        return $this;
    }
